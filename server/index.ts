...
namespace OpenESLintDocumentRequest {
...
class CodeActionResult {
  private readonly _actions: Map<string, RuleCodeActions>;
...
process.exit = ((code?: number): void => {
  ...
  connection.sendNotification(exitCalled, [code || 0, stack.stack]);
  setTimeout(() => {
    nodeExit(code);
  }, 1000);
}) as any;
...
function resolveSettings(
  ...
  resultPromise = connection.workspace
    ...
    .then((settings: TextDocumentSettings) => {
      ...
      promise = resolveModule("patched", directory, nodePath).catch(() => {
        return resolveModule("patched", directory, nodePath);
      });
      return promise.then(
        path => {
...
class BufferedMessageQueue {
  ...
  public registerRequest<P, R, E, RO>(
    ...
    this.connection.onRequest(type, (parameters, token) => {
  ...
  public registerNotification<P, RO>(
    ...
    connection.onNotification(type, parameters => {
      this.queue.push({
        method: type.method,
        params: parameters,
        documentVersion: versionProvider
...
connection.onInitialize(_parameters => {
  return {
    capabilities: {
      textDocumentSync: {
        openClose: true,
...
function getMessage(error, document: TextDocument): string {
  ...
  executeInWorkspaceDirectory(
    ...
    (file: string, options: CLIOptions) => {
      ...
      if (
        ...
        const documentReport = report.results[0];
        if (documentReport.messages && Array.isArray(documentReport.messages)) {
          documentReport.messages.forEach(problem => {
            if (problem) {
              const isWarning =
                convertSeverity(problem.severity) ===
              ...
              if (settings.autoFix) {
                if (
                  ...
                  if (rule.meta && typeof rule.meta.fixable === "string") {
      ...
      if (
        typeof cli.getRules === "function" &&
        !ruleDocumentData.handled.has(uri)
      ) {
        ruleDocumentData.handled.add(uri);
        cli.getRules().forEach((rule, key) => {
          if (rule.meta && rule.meta.docs && Is.string(rule.meta.docs.url)) {
            ruleDocumentData.urls.set(key, rule.meta.docs.url);
          }
        });
      }
    }
      ...
      if (error.messageTemplate === "plugin-missing") {
        connection.console.error(
          [
            ...
            `Happened while validating ${fsPath || document.uri}`,
            ...
            "",
            `Consider running eslint --debug ${fsPath ||
            document.uri} from a terminal to obtain a trace about the configuration files used.`
          ].join("\n")
        );
      } else {
        connection.console.error(
...
messageQueue.registerNotification(
  ...
  parameters => {
    ...
    parameters.changes.forEach(change => {
...
class Fixes {
  constructor(private readonly edits: Map<string, FixableProblem>) { }
...
messageQueue.registerRequest(
  ...
  parameters => {
    ...
    const { uri } = parameters.textDocument;
    ...
    return resolveSettings(textDocument).then(settings => {
      for (const editInfo of fixes.getScoped(parameters.context.diagnostics)) {
        documentVersion = editInfo.documentVersion;
        const { ruleId } = editInfo;
        ...
        if (
          ...
          if (ruleDocumentData.urls.has(ruleId)) {
...
messageQueue.registerRequest(
  ...
  async parameters => {
    ...
    if (parameters.command === CommandIds.applyAutoFix) {
      const identifier: VersionedTextDocumentIdentifier =
      ...
      [
        ...
        CommandIds.applyDisableLine,
        CommandIds.applyDisableFile
      ].includes(parameters.command)
    ) {
      const ruleId = parameters.arguments[0];
      workspaceChange = commands.get(`${parameters.command}:${ruleId}`);
    } else if (parameters.command === CommandIds.openRuleDoc) {
      const ruleId = parameters.arguments[0];
      const url = ruleDocumentData.urls.get(ruleId);
      if (url) {
        connection.sendRequest(OpenESLintDocumentRequest.type, { url });
      }
    } else {
      workspaceChange = commands.get(parameters.command);
    }
    ...
    try {
      ...
      if (!response.applied) {
        connection.console.error(
          `Failed to apply command: ${parameters.command}`
        );
      }
    }
    catch (error) {
