...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  const selector: DocumentSelector = filetypes.reduce(
    (res, filetype) =>
      res.concat([
  ...
  const clientOptions: LanguageClientOptions = {
    ...
    middleware: {
      ...
      workspace: {
        configuration: (parameters, _token, _next): any =>
  ...
  subscriptions.push(
    commands.registerCommand("eslint.executeAutofix", async () => {
      ...
      client
        .sendRequest(ExecuteCommandRequest.type, parameters)
        .then(undefined, () => {
          workspace.showMessage(
  ...
  client.onReady().then(
    () => {
      client.onNotification(exitCalled, parameters => {
        workspace.showMessage(
          `Server process exited with code ${parameters[0]}. This usually indicates a misconfigured ESLint setup.`,
          "error"
        );
      });
      client.onRequest(NoConfigRequest.type, parameters => {
        const document = Uri.parse(parameters.document.uri);
      ...
      client.onRequest(NoESLintLibraryRequest.type, parameters => {
        const uri: Uri = Uri.parse(parameters.source.uri);
        workspace.showMessage(
      ...
      client.onRequest(OpenESLintDocumentRequest.type, async parameters => {
        await commands.executeCommand("vscode.open", Uri.parse(parameters.url));
        return {};
      });
...
function getWorkspaceFolder(uri: string): WorkspaceFolder | null {
  ...
  const folder = workspace.workspaceFolders.find(o =>
    fsPath.startsWith(Uri.parse(o.uri).fsPath)
  );
  return folder;
}
